...
{
	// Protocol interfaces generated by bgen, declared here so they can be used in the API definitions.
	// Use them (instead of the protocol classes) where any object adopting the protocol is passed,
	// e.g. a GADInterstitialAd or GADRewardedAd passed as id<GADFullScreenPresentingAd>.
	interface IGADFullScreenPresentingAd { }
	interface IGADAdMetadataProvider { }
	interface IGADAdNetworkExtras { }

	// Symbols whose types [Field] can't bind (a C string and GADAdSize structs). Declaring them here makes
	// the native linker keep and export them; the public API reads them by address in Additions.cs.
	// The generated getters return the first pointer-sized bytes of each symbol and must not be used.
	[Static, Internal]
	interface NativeSymbols
	{
		...
		IntPtr GoogleMobileAdsVersionString { get; }
	...
	{
		// GoogleMobileAdsVersionString is a C string (const unsigned char[]), not an NSString.
		// See Constants.GoogleMobileAdsVersionString in Additions.cs.
		...
		// GADAdSize constants (GADAdSizeBanner, kGADAdSizeSmartBannerPortrait, ...) are structs, which
		// [Field] doesn't support. They are exposed as AdSize.Banner, AdSize.LargeBanner, ... in Additions.cs.
...
*/
	...
	interface GADRequest : INSCopying
	{
		...
		// @property (nonatomic, weak) UIWindowScene * _Nullable scene;
...
*/
	...
	interface GADAdMetadataDelegate
	{
		...
		void AdMetadataDidChange(IGADAdMetadataProvider ad);
	}
...
*/
	...
	interface GADFullScreenContentDelegate
	{
		...
		void AdDidRecordImpression(IGADFullScreenPresentingAd ad);
		...
		void AdDidRecordClick(IGADFullScreenPresentingAd ad);
		...
		void AdDidFailToPresentFullScreenContent(IGADFullScreenPresentingAd ad, NSError error);
		...
		void AdWillPresentFullScreenContent(IGADFullScreenPresentingAd ad);
		...
		void AdWillDismissFullScreenContent(IGADFullScreenPresentingAd ad);
		...
		void AdDidDismissFullScreenContent(IGADFullScreenPresentingAd ad);
	}
...
*/
	...
	interface GADRequestConfiguration
	{
		...
		[Export("ageRestrictedTreatment", ArgumentSemantic.Assign)]
		GADAgeRestrictedTreatment AgeRestrictedTreatment { get; set; }

		// @property (copy, nonatomic) NSNumber * _Nullable tagForUnderAgeOfConsent __attribute__((deprecated("Use the ageRestrictedTreatment property instead.")));
		...
		// @property (copy, nonatomic) NSNumber * _Nullable tagForChildDirectedTreatment __attribute__((deprecated("Use the ageRestrictedTreatment property instead.")));
		[Obsolete("Use AgeRestrictedTreatment instead.")]
